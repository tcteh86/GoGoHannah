...
def test_progress_report_csv_contains_summary_and_rows():
    child_name = 'report_child'
    client.post(
        '/v1/progress/exercise',
        json={
    ...
    response = client.get('/v1/progress/report.csv', params={'child_name': child_name})

    assert response.status_code == 200
    assert 'metric,value' in response.text
    assert 'total_exercises' in response.text
    assert 'word,exercise_type,score,correct,created_at' in response.text
    assert 'apple,quiz,90' in response.text
